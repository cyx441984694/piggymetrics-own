...
function initAccount(account){
    ...
    savings = new Savings(account.saving.amont, account.saving.deposit, account.saving.capitalization, account.saving.interest);
...
function initGreetingPage(){
    ...
    // Init statistics page first time
...
function greetingPageAgain() {
    $("$avatarcontainer").removeClass().addClass("foraward plus");
    $(".avatar").css({"background": "url(images/userpic.jpg) center center no-repeat", "background-size": "100% 100%"});
    $("#logo_greeting").fadeIn(0, function () {
...
function addItems() {
    ...
    //Markup changes accordin to number of column items
...
function itemsPosition(transaction){
    if ($("div$" + transaction + "slider").children().length<=4) {
        $("#" + transaction + "slider").css({"postion": "relative"});
    }
    else {
        $("#" + transaction + "slider").css({"postion":"absolute"});
...
function Up(transaction){
    ...
    if (Math.abs($("#" + transaction + "slider").position().top % itemWidth) > 0.01) return;
        ...
        var pixels=$("#" + transaction + "slider").position().top + (length - 5)* item;
        var slide = "translateY(" + pixels + "ps)";
...
function Down(transaction){
    ...
    if (Math.abs($("#" + transaction + "slider").position().top % itemWidth) > 0.01) return;
        ...
        if (transaction == 'expense'){
            ...
            setTimeout(startOfExpenseList, 500);
        }
        else{
            startOfIncomeList(transaction);
            setTimeout(startOfIncomeList, 500);
        }
    }
}
...
function addNewItem(){
    ...
    if (checkModalFields(itemVlaue,itemTitle)){
        if ($(".initicons").data("incomes-expenses") == "incomes"){
            ...
            addNewDiv("income",itemId,itemTitle,itemCurrency,itemPeriod,itemValue);
        }
        else{
        ...
        itemsPosition("expense");
        itemsPosition("income");
        setTimeout(function(){runCovert()},230);
}

function addNewDiv(whichColumn, itemId, itemIcon,itemCurrency, itemPeriod, itemValue){
...
function itemClick(item){
    ...
    if (itemDiv.hasClass("incomeitem")){
        ...
        $(".mainmodaltitle").empty().append("Cchange expense");
    }
...
//prepare calculation for 4 page
function runConvert(){
    expensesSumMonth =0;incomesSumMonth=0;
    $("#circle-select-1, #circle-select-2, #circle-select-3").empty();
    getConverted(incomes);
...
$("#noincomes, #noexpenses, .zoomplus, .plusitemborder").click(function(){
    ...
    if ($(this).hasClass("incomebutton")){
        ...
        $(".mainmodaltitle").apend("Add income");
    }
    else{
...
//MODAL: Sset zero when input is empty
...
//COLUMNS: mouse wheel
$('#expensewrapper').bind('DDOMMouseScroll mousewheel', function(e){
    if (e.origin.wheelDelta> 70){
        Down("expense");
    }
